...
const { isOpenGraphLink, generateOpenGraphUnfurl } = require('./opengraph');
...
async function linkShared(req, res) {
  ...
  // If we're sharing an opengraph.github.com link, and only that
  // then we can instantly respond with an unfurl to that URL as an image
  if (event.links.length === 1 && isOpenGraphLink(event.links[0].url)) {
    const link = event.links[0].url;
    const workspace = await SlackWorkspace.findOne({
      where: { slackId: req.body.team_id },
    });
    await workspace.client.chat.unfurl({
      ts: event.message_ts,
      channel: event.channel,
      unfurls: {
        [link]: generateOpenGraphUnfurl(link),
      },
    });
    return res.send();
  }
